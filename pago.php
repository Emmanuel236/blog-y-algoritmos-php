...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular el pago de la casa</h2>
		<form action="pago.php" method="post" class="casa-form">
			Sus ingresos: <input type="number" name="ingresos" required><br>
			<input type="submit" name="submit" value="Calcular pago de la casa" class="submit-button">
		</form>
		<?php
		if (isset($_POST['submit'])) {
			...
			if ($ingresos >= 800000) {
				$cuotaInicial = ($valorCasa * 30) / 100;
				$mensual = $valorCasa - $cuotaInicial;
				$total = $mensual / 84;
				echo "<p class='resultado'>Cuota inicial: $" . $cuotaInicial . "</p>";
				echo "<p class='resultado'>Mensualidad (84 meses): $" . $total . "</p>";
			} else {
				$cuotaInicial = ($valorCasa * 15) / 100;
				$mensual = $valorCasa - $cuotaInicial;
				$total = $mensual / 120;
				echo "<p class='resultado'>Cuota inicial: $" . $cuotaInicial . "</p>";
				echo "<p class='resultado'>Mensualidad (120 meses): $" . $total . "</p>";
			}
		}
		?>
	</div>
