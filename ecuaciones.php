...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Resolver ecuaciones ax+b=y</h2>
	<form action="ecuaciones.php" method="post" class="nota-form">
		a: <input type="number" name="a" required><br> 
		b: <input type="number" name="b" required><br>
		y: <input type="number" name="y" required><br>
		<input type="submit" name="submit" value="Resolver ecuación" class="submit-button">
	</form>
	<?php
	if (isset($_POST['submit'])) {
		...
		if ($a != 0) {
			$x = ($y - $b) / $a;
			echo "<p class='resultado'>La solución de la ecuación {$a}x + {$b} = {$y} es x = {$x}</p>";
		} else {
			echo "<p class='resultado'>No se puede resolver la ecuación porque 'a' no puede ser igual a cero.</p>";
		}
	} 
	?>
	</div>
