...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular compra de un artículo</h2>
	<form action="compra.php" method="post" class="venta-form">
		Valor de la Compra: <input type="number" name="compra" required><br>
		<input type="submit" name="submit" value="Calcular IVA y precio total" class="submit-button">
	</form>
	<?php
	if (isset($_POST['submit'])) {
		$compra = $_POST['compra']; 
		$iva = ($compra * 19) / 100;
		$total_con_iva = $compra + $iva;
		
		echo "<p class='resultado'>El IVA aplicado es: $" . $iva .  "</p>";
		echo "<p class='resultado'>El precio total con IVA es: $" . $total_con_iva .  "</p>";
	} 
	?>
	</div>
</body>
