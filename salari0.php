...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular el salario del trabajador</h2>
	<form action="salari0.php" method="post" class="salario-form">
		Horas trabajadas: <input type="number" name="horas_trabajadas" required><br>
		Horas extra trabajadas: <input type="number" name="horas_extra" required><br>
		<input type="submit" name="submit" value="Calcular salario" class="submit-button">
	</form>
	<?php
	if (isset($_POST['submit'])) {
		$horas_trabajadas = $_POST['horas_trabajadas']; 
		$horas_extra = $_POST['horas_extra'];
		$total = $horas_trabajadas * 15000;
		
		if ($horas_extra > 30) {
			$incremento = $total * 0.30;
			$total_pago = $total + $incremento;
		} else {
			$total_pago = $total;
		}
		
		echo "<p class='resultado'>El salario del trabajador es: $" . $total_pago .  "</p>";
	} 
	?>
	</div>
</body>
