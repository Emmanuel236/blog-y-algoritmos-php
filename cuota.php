...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular la cuota que debe pagar un cliente</h2>
		<form action="cuota.php" method="post" class="nota-form">
			Ingrese el monto:
			<input type="number" name="monto" required><br> 
			<input type="submit" name="submit" value="Calcular Cuota" class="submit-button">
		</form>
		<?php
		if (isset($_POST['submit'])) {
			...
			if ($monto < 500000) {
				$anadido = $monto * 0.03;
				$total = $monto + $anadido;
				echo "<p class='resultado'>El monto añadido es: " . $anadido .  "</p>";
				echo "<p class='resultado'>El total a pagar es: " . $total .  "</p>";
			} else {
				$anadido1 = $monto * 0.02;
				$total1 = $monto + $anadido1;
				echo "<p class='resultado'>El monto añadido es: " . $anadido1 .  "</p>";
				echo "<p class='resultado'>El total a pagar es: " . $total1 .  "</p>";
			}
		}
		?>
	</div>
