...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular el descuento de la promoción</h2>
		<form action="descuento.php" method="post" class="nota-form">
			Digite el valor de la compra:
			<input type="number" name="compra" required><br> 
			Digite el número para la Promoción:
			<input type="number" name="numero" required><br> 
			<input type="submit" name="submit" value="Calcular Descuento" class="submit-button">
		</form>
		<?php
		if (isset($_POST['submit'])) {
			...
			if ($numero >= 74) {
				$valor = ($compra * 20) / 100;
				$descuento = $compra - $valor;
				echo "<p class='resultado'>El valor de la compra es: $" . $compra .  "</p>";
				echo "<p class='resultado'>El descuento aplicado es del 20%.</p>";
				echo "<p class='resultado'>El valor después del descuento es: $" . $descuento .  "</p>";
			} else {
				$valor1 = ($compra * 15) / 100;
				$descuento1 = $compra - $valor1;
				echo "<p class='resultado'>El valor de la compra es: $" . $compra .  "</p>";
				echo "<p class='resultado'>El descuento aplicado es del 15%.</p>";
				echo "<p class='resultado'>El valor después del descuento es: $" . $descuento1 .  "</p>";
			}
		}
		?>
	</div>
