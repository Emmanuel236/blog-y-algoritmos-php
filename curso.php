...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular el valor a pagar por un curso</h2>
	<form action="curso.php" method="post" class="curso-form">
		Hora de compra:
		<input type="radio" name="hora" value="dia" required> Día
		<input type="radio" name="hora" value="noche" required> Noche
		<br>
		<input type="submit" name="submit" value="Calcular valor a pagar" class="submit-button">
	</form>
	<?php
	if (isset($_POST['submit'])) {
		...
		$porcentajeNoche = 5;
		if ($hora === 'noche') {
			$precioFinal = $precioBase + ($precioBase * $porcentajeNoche / 100);
		} else {
			$precioFinal = $precioBase;
		}
		echo "<p class='resultado'>El valor a pagar por el curso es: $" . $precioFinal .  "</p>";
	} 
	?>
	</div>
</body>
