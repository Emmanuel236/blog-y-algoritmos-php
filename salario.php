...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular el salario de una persona</h2>
	<form action="salario.php" method="post" class="nota-form">
		<input type="hidden" name="salario_minimo" value="1160000"> <!-- Salario mínimo en Colombia -->
		<input type="submit" name="submit" value="Calcular salario" class="submit-button">
	</form>
	<?php
	if (isset($_POST['submit'])) {
		$salario_minimo = $_POST['salario_minimo']; 
		$comisiones = 0.10 * $salario_minimo;
		$total_salario = $salario_minimo + $comisiones;
		echo "<p class='resultado'>El salario de la persona es: $" . number_format($total_salario, 2, ',', '.') . "</p>";
	} 
	?>
	</div>
</body>
