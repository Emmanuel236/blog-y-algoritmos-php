...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Hallar el costo de la matrícula</h2>
		<form action="matricula.php" method="post" class="matricula-form">
			Cantidad de materias: <input type="number" name="materias" required><br>
			<input type="submit" name="submit" value="Calcular matrícula" class="submit-button">
		</form>
		<?php
		if (isset($_POST['submit'])) {
			$materias = $_POST['materias'];
			$totalMat = $materias * 100000;
			
			if ($materias >= 5) {
				$descuento = ($totalMat * 30) / 100;
				$total = $totalMat - $descuento;
				echo "<p class='resultado'>Descuento aplicado: $" . $descuento . "</p>";
				echo "<p class='resultado'>Costo total de la matrícula: $" . $total . "</p>";
			} else {
				$iva = ($totalMat * 10) / 100;
				$total1 = $totalMat + $iva;
				echo "<p class='resultado'>IVA aplicado: $" . $iva . "</p>";
				echo "<p class='resultado'>Costo total de la matrícula: $" . $total1 . "</p>";
			}
		}
		?>
	</div>
