...
<body>
	...
	<nav class="menu">
		...
		<br>
	<div class="container">
	<h2>Calcular la nota total</h2>
	<form action="nota.php" method="post" class="nota-form">
		Trabajo (20%): <input type="number" name="trabajo" required><br> 
		Exposición (30%): <input type="number" name="exposicion" required><br>
		Ejercicios (40%): <input type="number" name="ejercicios" required><br>
		Gráfico (10%): <input type="number" name="grafico" required><br>
		<input type="submit" name="submit" value="Calcular nota total" class="submit-button">
	</form>
	<?php
	if (isset($_POST['submit'])) {
		...
		$notaTotal = ($trabajo * 0.2) + ($exposicion * 0.3) + ($ejercicios * 0.4) + ($grafico * 0.1); 
		echo "<p class='resultado'>La nota total es: " . $notaTotal .  "</p>";
	} 
	?>
	</div>
</body>
